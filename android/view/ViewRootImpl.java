...
public final class ViewRootImpl implements ViewParent,
    ...
    public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView,
        ...
        synchronized (this) {
            if (mView == null) {
                mView = view; // viewRootImpl 全局缓存 decorView
                ...
                // ViewRootImpl 持有 mAttachInfo, mAttachInfo 又持有 decorView
                // 所以 ViewRootImpl 和 decorView 是绑定关系
                ...
                // 在添加到窗口管理器之前,安排第一个布局
                // 确保我们在接收系统其它信号(像垂直同步信号)之前, 进行一次布局
                // requestLayout 先进行一次布局,会向消息队列发一个屏障消息
                requestLayout();
                ...
                try {
                    ...
                    // 请求 WMS 向底层 SurfaceFling 绘制
                    res = mWindowSession.addToDisplayAsUser(mWindow, mWindowAttributes,
                            getHostVisibility(), mDisplay.getDisplayId(), userId,
                            mInsetsController.getRequestedVisibilities(), inputChannel, mTempInsets,
                            mTempControls);
                    if (mTranslator != null) {
                        // 将屏幕内坐标转换为 InsetsState 应用窗口的坐标
    ...
    void scheduleTraversals() {
        if (!mTraversalScheduled) {
            ...
            // 屏障消息:向消息队列发送一个屏障消息,用来准备接收垂直同步信号的异步消息,准备更新 Ui 啦
            mTraversalBarrier = mHandler.getLooper().getQueue().postSyncBarrier();

            // 收到刷新信号后,处理消息队列的 异步消息, 顺便把 屏障消息 清掉
            // 远程回调信号传给 mTraversalRunnable
            // 监听垂直同步信号 ,当收到垂直同步信号时, 就会回调 mTraversalRunnable 的 run 方法, 然后调用里面的 doTraversal()
    ...
    void doTraversal() { // 收到垂直同步信号后执行
        if (mTraversalScheduled) { // 队列有屏障消息
            ...
            if (!mStopped || mReportNextDraw) {
                if (mWidth != host.getMeasuredWidth() || mHeight != host.getMeasuredHeight()
                    ...
                    if (DEBUG_LAYOUT) Log.v(mTag, "Ooops, something changed!  mWidth="
                     ...
                     // Ask host how big it wants to be 测量布局
                    performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
    ...
    void checkThread() {
        if (mThread != Thread.currentThread()) { // mThread 存的是 Main 线程,
